	...
	*p=newProcess;
	...
	else
	{
		...
		while(bb!=0)
		{
			...
			else if(p->curIoTime==bb->curIoTime)//Organize by name if remaining io time is the same.
			{
				if(p->name<bb->name)
				{
					if(bb==front)
...
{
	cout<<"IO Queue:     Process   IO Burst"<<endl;
	*outFile<<"IO Queue:     Process  IO Burst"<<endl;
	if(!Is_Empty())
	{
		node * p = front;
		while(p!=0)
